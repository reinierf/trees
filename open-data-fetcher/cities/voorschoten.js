...
const DATA_URL = 'https://openbomenkaart.org/data/trees_voorschoten.json';
...
function toTree(element) {
    ...
    // Diameter is an allometric estimate from species+age, e.g. "~0.25" (metres), not a measurement.
    const diameter = t.diameter != null ? parseFloat(String(t.diameter).replace('~', '').replace(',', '.')) : null;

    return {
        id:              String(element.id || t.admin_ref),
...
export default {
    ...
    layer: null,
    singleFetch: true,
    outputFile: { json: 'voorschoten.json', sqlite: 'voorschoten.db' },
