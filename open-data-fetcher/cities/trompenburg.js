#!/usr/bin/env node
/**
 * Fetches the Trompenburg Tuinen & Arboretum collection from the Von Gimborn
 * Arboretum's shared collection database (collectie.gimbornarboretum.nl).
 *
 * There is no API. This is a legacy ASP.NET WebForms + Telerik RadControls app
 * driven entirely by postbacks. Searching requires replaying its client/server
 * choreography:
 *
 *   1. GET  Zoeken.aspx?type=wetensch          → cookie + initial ViewState/EventValidation
 *   2. POST __EVENTTARGET=cbArboreta           → selects "Trompenburg Tuinen & Arboretum"
 *   3. POST __EVENTTARGET=cbSoortGewas         → selects growth form "WOODY"
 *   4. Per search term:
 ...
 *                                                 init_map(zoom,lat,lon,"TROMP",[markers])
 *
 * Every async postback response returns a *new* ViewState/EventValidation pair
 * (embedded in the response as `|hiddenField|__VIEWSTATECOMPRESSED|<value>|`), which
 * must be carried into the next request — reusing a stale pair produces corrupted,
 * inflated, or truncated results without any error. This was reverse-engineered from
 * a real browser HAR capture; see conversation history for the full trail.
 *
 ...
 * Trompenburg specialises in exotics registry.json won't know about), this fetches
 * once per letter a–z. Every specimen name necessarily contains at least one a–z
 * letter, so the union of 26 searches is complete by construction, with no
 * dependency on any pre-existing genus list.
 *
...
const ARBORETUM_INDEX   = 2; // "Trompenburg Tuinen & Arboretum" within cbArboreta
const GROWTH_FORM_INDEX = 0; // "WOODY" within cbSoortGewas
const ALPHABET = 'abcdefghijklmnopqrstuvwxyz'.split('');

// The cbArboreta checkbox has no effect on which specimens the search actually
// returns — confirmed empirically: selecting VGA (Doorn) vs TROMP (Rotterdam)
// for the same term returns byte-identical results, only the map's cosmetic
// label differs. The shared database's four institutions form four distinct,
// well-separated geographic clusters, so filter by Trompenburg's location
// instead — a small ~0.04° margin around its observed specimen cluster (also
// incidentally drops a handful of source records with corrupt/swapped coordinates).
const TROMPENBURG_BBOX = { latMin: 51.87, latMax: 51.93, lonMin: 4.50, lonMax: 4.61 };

function withinTrompenburg(lat, lon) {
    return lat >= TROMPENBURG_BBOX.latMin && lat <= TROMPENBURG_BBOX.latMax
        && lon >= TROMPENBURG_BBOX.lonMin && lon <= TROMPENBURG_BBOX.lonMax;
}
...
// sec-ch-ua produced the same generic error page as every other bug in this
// file, even with an otherwise byte-identical, correctly-chained request.
// Confirmed by diffing against a literal HAR capture with only these headers
// added. Not spoofing anything beyond what any browser sends by default.
...
function httpRequest(method, path, body, cookie, ajax = true) {
...
function clientListBox(checkedIndex) {
    return JSON.stringify({ isEnabled: true, logEntries: [], selectedIndices: [], checkedIndices: [checkedIndex], scrollPosition: 0 });
}
...
// renders a different control set than the default page — its buttons
// (plattegrond/lijst/afbeeldingen) and cbSoortGewas exist from first load,
// unlike the default page's autvolksnamen variant. Verified directly against
// a fresh fetch of this page (not by analogy from a different page variant).
function baseFields(session, term) {
    return {
        'ctl00_rwm1_ClientState': '',
        'ctl00$ddltaal': 'Nederlands',
        'ctl00_ddltaal_ClientState': '',
        'ctl00_Cp1_cbArboreta_ClientState': clientListBox(ARBORETUM_INDEX),
        'ctl00_Cp1_cbSoortGewas_ClientState': clientListBox(GROWTH_FORM_INDEX),
...
async function searchTerm(term, session) {
...
// Existing source vernacular names (Trompenburg's own "volksnaam") take
// priority; registry.json is only consulted to fill in what's missing.
...
function markerToTree(marker, fetchedAt) {
...
function rowToTree(row, fetchedAt) {
...
async function fetchTrompenburg({ includeUnmapped, letters }) {
    const fetchedAt = new Date().toISOString();
    process.stderr.write('[trompenburg] Establishing session (arboretum + growth-form filters)...\n');
    ...
    for (let i = 0; i < terms.length; i++) {
        ...
        const { markers, session: s1 } = await searchTerm(term, session);
        session = s1;
        for (const marker of markers) {
            if (!withinTrompenburg(marker.latitude, marker.longitude)) { droppedOtherArboretum++; continue; }
            const tree = markerToTree(marker, fetchedAt);
    ...
    if (includeUnmapped) {
        process.stderr.write('[trompenburg] Establishing a fresh session for List-view (unmapped specimen) pass...\n');
        session = await bootstrapSession();
        for (let i = 0; i < terms.length; i++) {
            const term = terms[i];
            drawProgress(i, terms.length);
            const { rows, session: s2 } = await listRows(term, session, msg => process.stderr.write(`[trompenburg] ${msg}\n`));
            session = s2;
            for (const row of rows) {
                const key = `${row.vaknummer}-${row.loopnummer}|${row.species}`;
                if (mappedKeys.has(key)) continue;
                if (byId.has(row.id)) continue;
                const tree = rowToTree(row, fetchedAt);
                if (!tree) { droppedSpecies++; continue; }
    ...
    process.stderr.write(`[trompenburg] Got ${trees.length} trees (${mappedCount} with coordinates${includeUnmapped ? `, ${unmappedCount} without` : ''}).\n`);
    if (droppedOtherArboretum > 0 || droppedSpecies > 0) {
        const total = droppedOtherArboretum + droppedSpecies;
        process.stderr.write(`[trompenburg] Dropped ${total} entries:\n`);
...
async function main() {
    ...
    const letters = args.term ? '\0' + args.term : args.letters; // '\0' prefix signals "don't split" to fetchTrompenburg
    const trees = await fetchTrompenburg({ includeUnmapped: args.includeUnmapped, letters });
    ...
    const outFile = args.output ?? path.join(DATA_DIR, args.format === 'json' ? 'trompenburg.json' : 'trompenburg.db');
    if (args.format === 'json') {
    ...
    process.stderr.write(`[trompenburg] Written to ${outFile}\n`);
}
